...
`nix-update` cannot regenerate the vendored `package-lock.json`, so a version bump whose upstream `package.json` changed a dependency range fails at `npm ci` with an out-of-sync lockfile.
That fails safe because the updater gates its push on `nix flake check`, but the fix is by hand: `npm install --package-lock-only` against the new tag, then a fresh `npmDepsHash`.
Upstream pins several of these deps exactly (`mermaid`, `react`, `@excalidraw/*`).
- Security advisories against the vendored lockfiles are fixed by hand, not by dependabot. Repo-level `dependabot_security_updates` is deliberately **disabled**: its npm fetcher needs a `package.json` beside each lockfile and these directories hold only `default.nix` + `package-lock.json`, so every run aborted with `Error during file fetching`; the job also runs `allowed-updates: direct` with `update-subdependencies: false`, and every advisory here lands on a transitive dep, so nothing would be eligible anyway. Alerts come from the dependency graph and still arrive. The fix is `npm update <pkg> --package-lock-only` against the upstream tag's `package.json`, then `nix-update --version=skip` for a fresh `npmDepsHash`.
Check `dev` in the lockfile entry first: `npmInstallHook` prunes devDependencies, so a dev-only advisory never reaches the built output. `lodash-es` in `lavish-axi` is the one that cannot move at all - `chevrotain` pins it exactly at `4.17.21`.
