...
That fails safe now that automated merges require a green build, but the fix is by hand: `npm install --package-lock-only` against the new tag, then a fresh `npmDepsHash`.
