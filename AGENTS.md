...
- Every timer-driven user service sets `OnFailure = [ "notify-failure@%n.service" ]` (`modules/home/notify-failure.nix`), which desktop-notifies with the last 5 journal lines. A sync that silently stopped working for weeks is the failure mode this exists to prevent - keep new units on it.
