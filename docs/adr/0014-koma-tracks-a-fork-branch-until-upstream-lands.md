...
Every other input in `flake.nix` points at a project's own repository, nearly all at a default branch and only `caelestia-dots/shell` at a release tag. This is the only one aimed at a personal fork branch, and `.nix` files here carry no comments (`0007-no-comments-in-nix.md`), so there is nowhere in the code to say that the fork is temporary or what ends it.
