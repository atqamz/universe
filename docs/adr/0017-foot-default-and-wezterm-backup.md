...
The split resolves in a single commit: the `configs/dotfiles` keybind change and the Universe terminal enablement land together in the same repository, so there is no cross-repository merge ordering to respect.
`configs/dotfiles` owns the keybinds and the presentation-shell launch action while Universe owns enablement and WezTerm behavior.
